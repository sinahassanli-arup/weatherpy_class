...
This script demonstrates how to import, unify, and clean weather data
using the WeatherPy library's dedicated classes for each operation.
"""
...
# Add the weatherpy_class directory to the path if needed
...
from weatherpy.data.wd_cleaner import WeatherDataCleaner
...
def process_bom_data():
    ...
    unified_data = unifier.unify(weather_data)
    ...
    # Step 3: Clean data using WeatherDataCleaner
    logger.info("Cleaning BOM data")
    cleaner = WeatherDataCleaner()
    cleaned_data = cleaner.clean(unified_data)
