...
def test_bom_import(station_id, year_start, year_end, time_zone, interval):
    ...
    print(f"\nTesting BOMWeatherDataImporter for station {station_id}...")
    try:
        # Create the importer
...
# Test the BOM import
bom_data = test_bom_import(station_id, year_start, year_end, time_zone, interval)

if bom_data is not None:
    print("\nImport successful!")
    print(f"Data shape: {bom_data.shape}")
    print(f"Data columns: {bom_data.columns.tolist()}")
    print(f"Data index: {bom_data.index.name}")
    print(f"Data index timezone: {bom_data.index.tz if hasattr(bom_data.index, 'tz') else 'None'}")
